...
struct Audio {
    ...
    bool init() {
        fprintf(stderr, "Calling MIX_Init...\n");
        if (!MIX_Init()) {
            fprintf(stderr, "MIX_Init error: %s\n", SDL_GetError());
            return false;
        }
        fprintf(stderr, "MIX_Init OK\n");

        fprintf(stderr, "Calling MIX_CreateMixerDevice...\n");
        ...
        if (!track_centipede || !track_spider ||
            !track_flea || !track_scorpion) {
        ...
        if (!track_shoot) {
            fprintf(stderr, "Error creating shoot track: %s\n", SDL_GetError());
            return false;
        }
        MIX_SetTrackAudio(track_shoot, shoot);

        // assign audio to looping tracks
        ...
        // set all looping tracks to loop forever
    ...
    void play_shoot() {
        if (!track_shoot) return;
        MIX_StopTrack(track_shoot, 0);
        MIX_SetTrackPlaybackPosition(track_shoot, 0);
        MIX_PlayTrack(track_shoot, 0);
    }
    void play_kill()  { if (kill)   MIX_PlayAudio(mixer, kill);  }
    void play_death() { if (death)  MIX_PlayAudio(mixer, death); }
    void play_bonus() { if (bonus)  MIX_PlayAudio(mixer, bonus); }
